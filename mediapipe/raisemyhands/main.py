...
def main():
    ...
    # カメラ設定
    cap = cv2.VideoCapture("output.mp4") # 0: 内蔵カメラ, 1: 外部カメラ　"output.mp4": 動画ファイル
    ...
    while(True):
        ...
        print("挙手判定: ", is_raise_hand(keypoints_list, scores_list, bbox_list))
...
# 挙手を判定する関数
def is_raise_hand(keypoints_list, scores_list, bbox_list):
...
def render(image, keypoints_list, scores_list, bbox_list):
    ...
    for i, (keypoints, scores, bbox) in enumerate(zip(keypoints_list, scores_list, bbox_list)):
        ...
        # 挙手判定
        is_raised = is_raise_hand([keypoints], [scores], [bbox])
